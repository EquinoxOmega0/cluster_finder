...
double precision, dimension(1:4) :: S_function
integer e,ee,eee,eeee,xyz
...
OPEN(78,file='log_opt.txt')

OPEN(77,file='quality_'//TRIM(appendix)//'/best_S_evolution.txt')
...
x_simplex(1,1)=b_factor_opt-(0.1/DBLE(xyz**2))
x_simplex(2,1)=R_factor_opt
! x_simplex(3,1)=s_power_opt+0.1
x_simplex(3,1)=lum_power_opt+(0.2/DBLE(xyz**2))

x_simplex(1,2)=b_factor_opt+(0.1/DBLE(xyz**2))
x_simplex(2,2)=R_factor_opt-(0.1/DBLE(xyz**2))
! x_simplex(3,2)=s_power_opt-0.05
x_simplex(3,2)=lum_power_opt-(0.2/DBLE(xyz**2))

x_simplex(1,3)=b_factor_opt
x_simplex(2,3)=R_factor_opt+(0.1/DBLE(xyz**2))
! x_simplex(3,3)=s_power_opt+0.1
x_simplex(3,3)=lum_power_opt-(0.1/DBLE(xyz**2))

x_simplex(1,4)=b_factor_opt
x_simplex(2,4)=R_factor_opt
! x_simplex(3,4)=s_power_opt
x_simplex(3,4)=lum_power_opt

! x_simplex(1,5)=b_factor_opt+0.2
! x_simplex(2,5)=R_factor_opt-0.2
! x_simplex(3,5)=s_power_opt-0.05
! x_simplex(4,5)=lum_power_opt-0.05
...
DO e=1,5
x_help(1)=x_simplex(1,e)
x_help(2)=x_simplex(2,e)
x_help(3)=x_simplex(3,e)
! x_help(4)=x_simplex(4,e)
...
DO ee=1,8
DO eee=1,8
IF (S_array(ee)>S_array(eee)) THEN
s_help=S_array(ee)
S_array(ee)=S_array(eee)
S_array(eee)=s_help
END IF
END DO
END DO
...
DO ee=2,4
IF ((S_function(highest))<(S_function(ee))) THEN
highest=ee
END IF
END DO

DO ee=2,4
IF ((S_function(second))<(S_function(ee))) THEN
IF (ee.NE.highest) THEN
second=ee
END IF
END IF
END DO
...
DO ee=2,4
IF ((S_function(lowest))>(S_function(ee))) THEN
lowest=ee
END IF
END DO

S_old_low=S_l
S_h=S_function(highest)
S_s=S_function(second)
S_l=S_function(lowest)



DO e=1,3
x_h(e)=x_simplex(e,highest)
END DO

DO e=1,3
x_l(e)=x_simplex(e,lowest)
END DO
...
! WRITE(*,*) 'iteration:',itercount
 com_change=0.D0
 DO e=1,3
 com_change=(x_c(e)-ax_c(e))**2
 END DO

 continueloop=.FALSE.
! 
! IF (com_change>0.01) THEN
!  continueloop=.TRUE.
! END IF
 ...
 DO ee=1,4
 DO e=1,3
 com_change=(x_simplex(e,ee)-x_c(e))**2
 END DO
 END DO
...
!  DO e=1,4
!  IF (((x_c(e)-ax_c(e))**2)>1.D-2) THEN
!  continueloop=.TRUE.
! END IF
!  END DO

! DO e=1,4
! IF (((x_oldlow(e)-x_l(e))**2)>1.D-6) THEN
!  continueloop=.TRUE.
! END IF
! END DO
...
DO e=1,3
err_X(e)=(x_h(e)-x_l(e))
errorsum=errorsum+(err_X(e))**2
END DO

DO e=1,3
IF (((err_X(e))**2)>0.0001) THEN
 continueloop=.TRUE.
END IF
END DO

IF (errorsum>0.0001) THEN
 continueloop=.TRUE.
END IF

IF ((S_old_low-S_l)>0.0001) THEN
 continueloop=.TRUE.
END IF
...
WRITE(77,*) b_factor_opt,R_factor_opt,lum_power_opt,S_tot_opt,&
err_X(1),err_X(2),err_X(3),errorsum,itercount
...
DO e=1,3
x_alpha(e)=(1.D0+simplex_alpha)*x_c(e)-simplex_alpha*x_h(e)
IF (x_alpha(e)<0.D0) THEN
x_alpha(e)=-x_alpha(e)
END IF
END DO
...
uuu=DBLE(u_local(1))
S_local(1)=calculategroupcostfunction(x_alpha,uuu)
...
DO e=1,3
x_gamma(e)=(1.D0-simplex_gamma)*x_c(e)+simplex_gamma*x_alpha(e)
IF (x_gamma(e)<0.D0) THEN
x_gamma(e)=-x_gamma(e)
END IF
END DO
...
uuu=DBLE(u_local(1))
S_local(1)=calculategroupcostfunction(x_gamma,uuu)
...
IF (S_gamma<S_alpha) THEN

! S^k+1=gamma(alpha(S^k))
WRITE(*,*) 'gamma',itercount
WRITE(78,*) 'gamma',itercount
DO e=1,3
x_simplex(e,highest)=x_gamma(e)
END DO
S_function(highest)=S_gamma


ELSE

WRITE(*,*) 'alpha1',itercount
WRITE(78,*) 'alpha1',itercount
! S^k+1=alpha(S^k)
DO e=1,3
x_simplex(e,highest)=x_alpha(e)
END DO
S_function(highest)=S_alpha
...
! S^k+1=alpha(S^k)
WRITE(*,*) 'alpha2',itercount
WRITE(78,*) 'alpha2',itercount
DO e=1,3
x_simplex(e,highest)=x_alpha(e)
END DO
S_function(highest)=S_alpha
...
IF (S_alpha>S_h) THEN
DO e=1,3
x_beta(e)=(1.D0-simplex_beta)*x_c(e)+simplex_beta*x_h(e)
IF (x_beta(e)<0.D0) THEN
x_beta(e)=-x_beta(e)
END IF
END DO
ELSE
DO e=1,3
x_beta(e)=(1.D0-simplex_beta)*x_c(e)+simplex_beta*x_alpha(e)
IF (x_beta(e)<0.D0) THEN
x_beta(e)=-x_beta(e)
END IF
END DO
...
uuu=DBLE(u_local(1))
S_local(1)=calculategroupcostfunction(x_beta,uuu)
...
WRITE(*,*) 'delta',itercount
WRITE(78,*) 'delta',itercount
! S^k+1=delta(S^k)

DO ee=1,4
IF (ee.NE.lowest) THEN
DO e=1,3
x_simplex(e,ee)=x_l(e)+simplex_delta*(x_simplex(e,ee)-x_l(e))
END DO
...
x_help(1)=x_simplex(1,ee)
x_help(2)=x_simplex(2,ee)
x_help(3)=x_simplex(3,ee)
! x_help(4)=x_simplex(4,ee)

DO eee=1,3
IF (x_simplex(eee,ee)<0.D0) THEN
x_simplex(eee,ee)=0.D0
END IF
END DO
...
! S^k+1=beta(S^k)
WRITE(*,*) 'beta',itercount
WRITE(78,*) 'beta',itercount
DO e=1,3
x_simplex(e,highest)=x_beta(e)
END DO
S_function(highest)=S_beta
...
OPEN(50,file='quality_'//TRIM(appendix)//'/bestfit.txt')
WRITE(50,*) b_factor_opt,R_factor_opt,lum_power_opt
WRITE(50,*) err_X(1),err_X(2),err_X(3)!,err_X(4)
WRITE(50,*) S_tot_opt,itercount
CLOSE(50)
...
!  n_multi_cut=5
 ...
 OPEN(50,file='fit_lumass.txt')
READ(50,*) mfit_a,mfit_b,mfit_c
READ(50,*)
READ(50,*)
 CLOSE(50)
 
  OPEN(50,file='globalparameters.txt')
READ(50,*) basic_link
READ(50,*)
...
OPEN(50,file='quality_'//TRIM(apx)//'/binclusterfof_'//TRIM(nm)//'_'//TRIM(ecs)//'.txt')
DO i=1,102
...
OPEN(50,file='quality_'//TRIM(apx)//'/binclustermock_'//TRIM(nm)//'_'//TRIM(ecs)//'.txt')
DO i=1,102
...
!   CLOSE(50)
 ...
 OPEN(50,file='quality_'//TRIM(apx)//'/n_g_multi_'//TRIM(nm)//'_'//TRIM(ecs)//'.txt')
...
OPEN(50,file='quality_'//TRIM(apx)//'/group_cost_function_'//TRIM(nm)//'_'//TRIM(ecs)//'.txt')
