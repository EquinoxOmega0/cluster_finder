...
integer(kind=8) :: n_l,n_fl,calibrationmode,n_truegroups_mock,n_truegroups_all,n2plus_truemock,n2plus_trueall,n_ct
logical :: flipvar
...
logical, allocatable :: active(:),fof(:),fof_recursive(:),iter_help(:),tully_active(:)
...
double precision, allocatable :: lumdistance_clustercore(:),mass_clustercore(:),colour_clustercore(:),v_cc(:)
...
double precision, dimension(1:8) :: mass_coeff_ml
double precision, dimension(1:9) :: mass_coeff_mh
...
! IF (active(i)) THEN
! ! WRITE(*,*) vred(i)
! IF (vred(i)<3000.D0) THEN
! active(i)=.FALSE.
! END IF
! 
! IF (vred(i)>10000.D0) THEN
! active(i)=.FALSE.
! END IF
! 
! END IF
...
OPEN(50,file='compare_tully/linking_conditions.txt')
DO i=1,n
IF (active(i)) THEN
...
mass_clustercore(i)=mass_clustercore(i)+mass_coeff_ml(6)*(loglumdist**3)
mass_clustercore(i)=mass_clustercore(i)+mass_coeff_ml(7)*logmdyn+mass_coeff_ml(8)
...
mass_clustercore(i)=mass_clustercore(i)+mass_coeff_mh(6)*(loglumdist**3)+mass_coeff_mh(7)*logmdyn
mass_clustercore(i)=mass_clustercore(i)+mass_coeff_mh(8)*logmember+mass_coeff_mh(9)
...
DO i=1,counter
tully_active(i)=.TRUE.
v_cc(i)=((((1.D0+z_clustercore(i))**2)-1.D0)/(((1.D0+z_clustercore(i))**2)+1.D0))*light

IF (v_cc(i)<3000.D0) THEN
tully_active(i)=.FALSE.
END IF

IF (v_cc(i)>10000.D0) THEN
tully_active(i)=.FALSE.
END IF

IF (tully_active(i)) THEN
n_used=n_used+members_clustercore(i)
n_ct=n_ct+1
END IF
...
OPEN(50,file='compare_tully/cluster_list_all_'//TRIM(appendix)//'.txt')
DO i=1,counter
IF (tully_active(i)) THEN
...
OPEN(50,file='compare_tully/cluster_list_single_'//TRIM(appendix)//'.txt')
DO i=1,counter
IF (tully_active(i)) THEN
IF (members_clustercore(i)==1) THEN
WRITE(50,*) i,ra_clustercore(i),dec_clustercore(i),z_clustercore(i),&
...
OPEN(50,file='compare_tully/cluster_list_multi_'//TRIM(appendix)//'.txt')
DO i=1,counter
IF (tully_active(i)) THEN
IF (members_clustercore(i)>1) THEN
WRITE(50,*) i,ra_clustercore(i),dec_clustercore(i),z_clustercore(i),&
...
END DO
 ...
 OPEN(50,file='compare_tully/cluster_list_massive_'//TRIM(appendix)//'.txt')
DO i=1,counter
IF (tully_active(i)) THEN
...
OPEN(50,file='compare_tully/cluster_members_'//TRIM(appendix)//'.txt')
DO i=1,n
IF (oldact(i)) THEN
 IF (TRIM(appendix)=='SDSS') THEN
...
OPEN(50,file='compare_tully/generaldata_'//TRIM(appendix)//'.txt')
WRITE(50,*) 'number of clusters:',n_ct
...
OPEN(50,file='compare_tully/binclusterfof_'//TRIM(appendix)//'.txt')
DO i=1,102
