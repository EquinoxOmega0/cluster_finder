...
real, allocatable ::  cModelMag_g(:),cModelMag_r(:),cModelMag_i(:)
real, allocatable ::  extinction_g(:),extinction_r(:),extinction_i(:)
real, allocatable :: cModelMagErr_g(:),cModelMagErr_r(:),cModelMagErr_i(:)
...
real, allocatable :: K_g(:),K_r(:),K_i(:)

 real, allocatable :: magapp_g(:),magapp_r(:),magapp_i(:)
 ...
 real, allocatable :: magabs_g(:),magabs_r(:),magabs_i(:)
...
OPEN(50,file='SDSS_real/Prajwal_fullSDSS.txt')
DO i=1,n_sdss
IF (sdss_active(i)) THEN
WRITE(50,*) sdss_objID(i),sdss_ra(i),sdss_dec(i),z_cor(i),magapp_g(i),magapp_r(i),magapp_i(i)
END IF
END DO
